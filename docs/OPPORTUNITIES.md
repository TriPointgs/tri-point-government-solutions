...
1. Create an opportunity or paste structured JSON into **Import opportunity JSON**. Both the opportunity format (`title`, `requirements`, `internal`, `review`, `disclosure`) and the requisition format (`requisition_id`, `solicitation`, `scope`, `financials`, `compliance`, `tri_point_review`) are supported. Import only loads a review form; save it explicitly. An empty solicitation title can use the requisition ID as a temporary internal title; replace it after reviewing the solicitation. Requisition content goes into private fields, with outreach disclosure left blank. Imported contractor selections, drafts, delivery flags and approvals are ignored. Review import notes, trade/geography and separate prime versus subcontractor obligations before saving. It is not an automated solicitation/PDF parser or opportunity scraper. Read the complete solicitation, attachments, pricing schedules and addenda first.
