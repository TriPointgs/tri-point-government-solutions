...
Build runs Node unit tests and static-page metadata/internal-link checks. Before driving traffic, complete actual form persistence, authenticated reads, unauthenticated denial and test-record deletion. The release preview passed its 20 automated unit tests and static metadata/link checks. The automated browser/API submission test was blocked by the execution environment, so it has NOT established end-to-end persistence or admin-session behavior. Owner browser acceptance remains required; do not call the form end-to-end verified until that check passes.
