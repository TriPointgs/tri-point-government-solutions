import {get,head,put,list,BlobNotFoundError} from '@vercel/blob';
...
async function read(kind,id) {
 ...
 let before;
 try{before=await head(path);}catch(error){
  if(error instanceof BlobNotFoundError)throw new HttpError(404,'The requested private record was not found.');
  throw error;
 }
